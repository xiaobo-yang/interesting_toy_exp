...
class GPT(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, model_type):  # hook是sae实验所需，获取所有mlp层的输出
    ...
    @torch.no_grad()
    def generate(self, idx, max_new_tokens, do_sample, temperature=1.0, top_k=None, eot_token=50256):
        orig_idx = idx.clone()
        ...
        # 处理生成的序列，将第一个EOT标记之后的所有标记替换为EOT标记
        B = idx.size(0)  # 批次大小
        for i in range(B):
            eot_positions = (idx[i] == eot_token).nonzero(as_tuple=True)[0]
            if len(eot_positions) > 0:
                first_eot_pos = eot_positions[0]
                if first_eot_pos >= len(orig_idx[i]):  # 排除prompt对齐时左边pad的token
                    idx[i, first_eot_pos+1:] = eot_token
